...
const isFileUrl = function (file) {
  return /^file:\/\//i.test(file)
}

const formattedImport = async (file, esmDecorator = forward) => {
  if (!isFileUrl(file) && path.isAbsolute(file)) {
    try {
...
const tryImportAndRequire = async (file, esmDecorator) => {
  if (path.extname(file) === ".mjs" || isFileUrl(file)) {
...
exports.loadFilesAsync = async (
  ...
  for (const file of files) {
    preLoadFunc(file);
    resolvedFile = isFileUrl(file) ? file : path.resolve(file);
